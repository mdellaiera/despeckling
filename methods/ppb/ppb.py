...
class PPB(BaseFilter):
    """Wrapper around PPB filter."""
    ...
    def filter(
            ...
            alpha: float,
            T: float,
            nbits: int) -> Union[np.ndarray, jnp.ndarray]:
        ...
        if isinstance(sar, jnp.ndarray):
            output = jnp.array(y).reshape(amplitude.shape)
        else:
            output = np.array(y).reshape(amplitude.shape) 
