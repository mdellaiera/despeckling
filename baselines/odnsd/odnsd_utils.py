...
class ODUBF:
    """Optical-Driven Unnormalized Bilateral Filter (ODUBF)."""
...
class SoftClassifier:
    ...
    def compute_weight(self, sar: jnp.ndarray, N: List[int], gamma: float = 7, a0: float = 0.64) -> jnp.ndarray:
        ...
        N : List[int]
            List of look numbers for each class.
        gamma : float
...
class SARDespeckling:
    ...
    def filter(self, 
        ...
        logging.info("Applying ODUBF...")
        self.uO = odubf.filter(sar, opt, window_size, lambda_S, lambda_RO, lambda_RS)
